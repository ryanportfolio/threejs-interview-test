...
- Browser per session, never shared. The official playwright plugin holds one persistent profile; a second connection fails with "Browser is already in use ... use --isolated" and deadlocks. Parallel or subagent browser work uses the `playwright-iso` MCP server from `.mcp.json` (`@playwright/mcp --isolated`, in-memory profile).
